...
class AirDockingEnv(gym.Env):
    ...
    def change_goal(self,):
        if self.env.step_count > 300 and not np.any(self.env.dynamics.contact_force) and self.change_goal_flag:
            self.vel_ += 0.001
    ...
    def contactRatio(self,):
        ...
        if not self.env.crashed or self.env2.crashed:
            ...
            # print(f"p1_y-p2_y is : {p1_y-p2_y}")
                ...
                #pdb.set_trace()
                    if p2_y - p1_y >= depth_tau-0.025 and self.dis_judge >= 200:
    ...
    def reset(self,):
        ...
        if self.control_name == "thrustOmegaPolicy3":
            #random_att
            rot1 = randomRot(np.pi/20, np.pi/20)
            self.env.quadrotor.setBaseOrientation(rot1)

            rot2 = randomRot(np.pi/20, np.pi/20)
            self.env.quadrotor.setBaseOrientation(rot2)
        else:
            #random_att
            rot1 = randomRot(0, np.pi/10)
            self.env.quadrotor.setBaseOrientation(rot1)

            rot2 = randomRot(0, np.pi/10)
            self.env.quadrotor.setBaseOrientation(rot2)
...
if __name__ == '__main__':
    ...
    env = AirDockingEnv(rew_coeff=rew_coeff_sou)
    state = env.reset()
    for i in range(1000):
