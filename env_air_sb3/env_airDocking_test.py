...
class airDockingTest(gym.Env):
    ...
    def __init__(self,file_1="../rsc/AirDocking_6/airDocking.urdf",
        ...
        file_path = os.getcwd()
        self.env = quadrotorEnv(file_path+ file_1, name="F450", 
                                ...
                                max_step = max_step,
                                dt = dt)
        self.env2 = quadrotorEnv(file_path+file_2, name="F450_2", 
    ...
    def step(self, action=np.zeros(8)):
        ...
        if self.train_type == "double":
            ...
            if self.control_name == "velocityYaw":
                reward, info = self.getRewardRatio()
            else:
        ...
        if self.control_name == "pidThrustOmega" and not done:
            if self.change_goal():
                ...
                self.success_flag = done

        elif self.control_name == "forceThrustOmega" and not done:
            if self.change_goal():
                done = self.find_pattern_done()
                self.success_flag = done
        else:
    ...
    def contactRatio(self,):
        ...
        # 套筒goal位置
        goal1 = self.env.dynamics.pos +  tau1_xyz
        goal2 = self.env.dynamics.pos + np.array([0,-0.31,-0.03])
        ...
        # a1 = self.env.dynamics.rot @ tau1_xyz
        
        # a1 = a1 / np.linalg.norm(a1)
        
        # a2 = self.env2.dynamics.rot @ tau2_xyz
        
        # a2 = a2 / np.linalg.norm(a2)        
        # print('tau1_xyz_world', a1)
        # print('tau2_xyz_world', a2)
        ...
        if not self.env.crashed or self.env2.crashed:
            ...
            # print(f"p1_y-p2_y is : {p1_y-p2_y}")
                ...
                #pdb.set_trace()
                    if p2_y - p1_y >= depth_tau-0.025: #and self.dis_judge > 300:
    ...
    def reset(self,):
        ...
        pos2[1] = goal2[1] + random.uniform(-0.5, -0.5)
        ...
        # pos2[1] = goal2[1] + -1            #left
        # pos2[0] = goal2[0] + 0.3             #back
        # pos2[2] = goal2[2] + 0.1           #up
        ...
        if self.control_name == "thrustOmegaPolicy3":
            #random_att
            rot1 = randomRot(0, np.pi/30)
            self.env.quadrotor.setBaseOrientation(rot1)

            # rot2 = randomRot(0, np.pi/20)
...
if __name__ == '__main__':
    env = airDockingTest(rew_coeff=rew_coeff_sou)
    state = env.reset()
    for i in range(1000):
        ...
        print(f"uav1 {env.env.position_w}")
        print(f"uav2 {env.env2.position_w}")
        time.sleep(0.05)
        if done:
            break
